...
HX_STACK_PUSH("Display::new","example/Display.hx",25);
{
	...
	HX_STACK_LINE(8)
	::Sys_obj::println(HX_CSTRING("    CPP: create new Display"));
}
;
	return null();
}
...
Void Display_obj::redraw( ){
{
		...
		HX_STACK_LINE(14)
		::String str = ((((((((HX_CSTRING("{x:") + this->x) + HX_CSTRING(", y:")) + this->y) + HX_CSTRING(", width:")) + this->width) + HX_CSTRING(", height:")) + this->height) + HX_CSTRING("}"));		HX_STACK_VAR(str,"str");
		HX_STACK_LINE(15)
		::Sys_obj::println((HX_CSTRING("    CPP: redraw ") + str));
	}
return null();
}

...
class TypeTools {
	...
	public static function getClassFields(t:ClassType, out:Array<Field>, marker:Hash<Bool>)
	{
		var fields = t.fields.get();

		getClassFields2(t, fields, out, marker);

	}

	public static function getClassFields2(t:ClassType, fields:Array<ClassField>, out:Array<Field>, marker:Hash<Bool>) {
		for (field in fields) 
								...
								default: 
									switch (reduce(field.type)) {
										case TFun(args, _):
											var argList = [];
											for (arg in args)
												argList.push(FunctionTools.toArg(arg.name, makeBlankType(field.pos)));
											FFun({
												args : argList,
												ret: makeBlankType(field.pos),
												expr: null,
												params: []
											});
											//null;
